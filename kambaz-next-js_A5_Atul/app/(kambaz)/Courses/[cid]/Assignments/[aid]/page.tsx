...
The Kanbas application should include a link to navigate back to the landing page.`,
    ...
    // Piyush’s extra fields, but with your semantics/defaults
  ...
  // 🔄 NEW from Piyush: fetch assignments from server on mount
  const fetchAssignments = async () => {
    try {
      ...
      // If editing, refresh current assignment from server data
      if (!isNew) {
  ...
  const handleSave = async () => {
    const payload = {
      ...assignment,
      course: cid,
      // compose friendly strings like you originally did
    ...
    try {
      if (isNew) {
        // NEW: create on server, but still use your Redux addAssignment
        ...
        // NEW: update on server, then your Redux updateAssignment
        const updatedAssignment = await client.updateAssignment(payload);
